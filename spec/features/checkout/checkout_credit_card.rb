...
RSpec.describe 'Credit card in checkout flow', type: :feature, js: true, vcr: true do
  ...
  context 'with existing credit card' do
    ...
    context 'with multiple credit cards' do
      ...
      let!(:credit_card) { create(:credit_card, user: user, payment_method: stripe_card_payment_method) }
      let!(:different_credit_card) { create(:credit_card, user: user, payment_method: stripe_card_payment_method) }
      ...
      it 'chooses different card' do
        ...
        page.execute_script("$(\"label[for='order_existing_card_id_#{different_credit_card.id}']\").trigger('click')")
