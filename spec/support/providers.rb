RSpec.configure do |config|
  config.before do
    ...
    allow_any_instance_of('Spree::Inventory::Providers::GenericMetadataProvider'.constantize).to receive(:images).and_return([])

    %w[Music].each do |type|
  ...
  config.before(:each, images: true) do
    ...
    allow_any_instance_of('Spree::Inventory::Providers::GenericMetadataProvider'.constantize).to receive(:images).and_call_original
