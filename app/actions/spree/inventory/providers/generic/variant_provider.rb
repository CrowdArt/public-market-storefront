      ...
      module Generic
        class VariantProvider < DefaultVariantProvider
          ...
          def find_metadata(_identifier)
            MetadataProvider.call(item_json.stringify_keys, VALIDATION_SCHEMA.rules.keys, taxonomy_name: taxonomy_name)
          end

          def categorize(product, taxons)
            Classifier.call(product, taxons.presence || original_taxons, taxonomy_name: taxonomy_name)
          end
